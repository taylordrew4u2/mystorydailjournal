...
struct EntryView: View {
    ...
    private func content(for record: DayRecord) -> some View {
        VStack(spacing: 0) {
            if record.source == .autoGenerated, panel == .diary {
                AutoDayBanner(generatedDiaryEnabled: settings.generatedDiaryEnabled)
                    .padding(.top, 8)
            }
            ...
            case .diary:
                if isGeneratingPastDay {
                    Spacer()
                    ProgressView(settings.generatedDiaryEnabled ? "Writing your day..." : "Collecting facts...")
                    Spacer()
                } else if shouldShowFactsOnly(for: record) {
                    FactsCollectedView(record: record)
                } else {
    ...
    private func shouldShowFactsOnly(for record: DayRecord) -> Bool {
        !settings.generatedDiaryEnabled && record.source == .autoGenerated
    }
...
private struct AutoDayBanner: View {
    let generatedDiaryEnabled: Bool
...
private struct FactsCollectedView: View {
    let record: DayRecord

    private var facts: [FactRow] {
        FactRowsBuilder.rows(
            from: record.signals ?? [],
            timeZoneIdentifier: record.timeZoneIdentifier
        )
    }
        ...
        List {
            if facts.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text("Turn on signals or add notes, photos, and files from the menu.")
                )
            } else {
                Section {
                    ...
                    Text("Generated diary writing is off. These facts stay private and can still sync through iCloud when available.")
                }
            }
        }
...
private enum FactRowsBuilder {
    static func rows(from signals: [DaySignal], timeZoneIdentifier: String) -> [FactRow] {
        var calendar = Calendar.current
        calendar.timeZone = TimeZone(identifier: timeZoneIdentifier) ?? .current

        return signals
            .sorted { $0.timestamp < $1.timestamp }
            .compactMap { row(for: $0, calendar: calendar) }
    }

    private static func row(for signal: DaySignal, calendar: Calendar) -> FactRow? {
        let time = timeString(for: signal.timestamp, calendar: calendar)
        switch signal.kind {
        case .visit:
            guard let payload = signal.payload(as: VisitPayload.self), !payload.isPassingThrough else { return nil }
            let label = payload.categoryLabel.map { " \($0)" } ?? ""
            return FactRow(id: id(signal), title: "Place", detail: "\(payload.placeName)\(label) at \(time)")
        case .photo:
            guard let payload = signal.payload(as: PhotoPayload.self) else { return nil }
            var parts = [payload.isScreenshot ? "Screenshot" : "Photo"]
            if let placeName = payload.placeName { parts.append(placeName) }
            if !payload.sceneLabels.isEmpty { parts.append("Labels: \(payload.sceneLabels.prefix(2).joined(separator: ", "))") }
            if payload.faceCount > 0 { parts.append(faceSummary(payload)) }
            return FactRow(id: id(signal), title: "Photo", detail: parts.joined(separator: " - ") + " at \(time)")
        case .calendar:
            guard let payload = signal.payload(as: CalendarPayload.self) else { return nil }
            let location = payload.location.map { " at \($0)" } ?? ""
            return FactRow(id: id(signal), title: "Calendar", detail: "\"\(payload.title)\"\(location), \(time)")
        case .activity:
            guard let payload = signal.payload(as: ActivityPayload.self) else { return nil }
            return FactRow(id: id(signal), title: "Activity", detail: activitySummary(payload))
        case .media:
            guard let payload = signal.payload(as: MediaPayload.self), !payload.titles.isEmpty else { return nil }
            return FactRow(id: id(signal), title: "Music", detail: payload.titles.prefix(3).joined(separator: ", "))
        case .weather:
            guard let payload = signal.payload(as: WeatherPayload.self) else { return nil }
            return FactRow(id: id(signal), title: "Weather", detail: weatherSummary(payload))
        case .sharedItem:
            guard let payload = signal.payload(as: SharedItemPayload.self) else { return nil }
            let title: String
            if let itemTitle = payload.title, !itemTitle.isEmpty {
                title = itemTitle
            } else {
                title = "Saved item"
            }
            return FactRow(id: id(signal), title: title, detail: snippet(payload.text))
        case .fileWatch:
            guard let payload = signal.payload(as: FileWatchPayload.self) else { return nil }
            return FactRow(id: id(signal), title: "File", detail: "\(payload.fileName) in \(payload.folderName)")
        case .attachment:
    ...
    private static func id(_ signal: DaySignal) -> String {
        "\(signal.kind.rawValue)-\(signal.timestamp.timeIntervalSinceReferenceDate)"
    }
    ...
    private static func activitySummary(_ payload: ActivityPayload) -> String {
        var parts: [String] = []
        if payload.stepCount > 0 {
            parts.append("\(payload.stepCount.formatted()) steps")
        }
        if payload.distanceMeters > 0 {
            parts.append(String(format: "%.1f km", payload.distanceMeters / 1000))
        }
        parts.append(contentsOf: payload.workoutSummaries)
        if payload.sleepHours > 0 {
            parts.append(String(format: "%.1f hours sleep", payload.sleepHours))
        }
        return parts.isEmpty ? "No activity details" : parts.joined(separator: ", ")
    }

    private static func weatherSummary(_ payload: WeatherPayload) -> String {
        var parts = [payload.conditionDescription]
        if let high = payload.highTemperatureCelsius {
            parts.append("high \(Int(high.rounded()))°")
        }
        if let low = payload.lowTemperatureCelsius {
            parts.append("low \(Int(low.rounded()))°")
        }
        return parts.joined(separator: ", ")
    }

    private static func attachmentRow(_ payload: AttachmentPayload, id: String) -> FactRow? {
        switch payload.kind {
        case .note:
            return FactRow(id: id, title: "Note", detail: snippet(payload.text ?? ""))
        case .photo:
            return FactRow(id: id, title: "Attached photo", detail: payload.assetLocalIdentifier ?? "")
        case .file:
            return FactRow(id: id, title: "Attached file", detail: payload.fileName ?? "")
        }
    }
